...
class MainWindow:
    ...
    def _create_control_panel(self, parent):
        ...
        control_frame.grid(row=0, column=1, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # Recognition method selection
        ttk.Label(control_frame, text="Recognition Method:").pack(pady=5)
        
        lbph_radio = ttk.Radiobutton(control_frame, text="LBPH (Fast)", 
                                     variable=self.current_method, value='lbph',
                                     command=self._on_method_change)
        lbph_radio.pack(anchor=tk.W)
        
        openface_radio = ttk.Radiobutton(control_frame, text="OpenFace (Accurate)", 
        ...
        haar_radio = ttk.Radiobutton(control_frame, text="Haar Cascade", 
                                     variable=self.current_detection, value='haar',
                                     command=self._on_detection_change)
        haar_radio.pack(anchor=tk.W)
        
        dnn_radio = ttk.Radiobutton(control_frame, text="DNN", 
                                   variable=self.current_detection, value='dnn',
                                   command=self._on_detection_change)
        ...
        self.threshold_label = ttk.Label(control_frame, text=f"{self.threshold_var.get():.1f}")
        self.threshold_label.pack()
        
        threshold_scale = ttk.Scale(control_frame, from_=0, to=100,
        ...
        # View logs button
        logs_button = ttk.Button(control_frame, text="View Access Logs", 
    ...
    def _create_status_panel(self, parent):
        ...
        status_frame.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.W, tk.E))
        ...
        while self.is_running:
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.putText(frame, label, (x, y-10),
                          config.FONT_FACE, config.FONT_SCALE, color, config.FONT_THICKNESS)
                
                # Log access (với cooldown)
                current_time = time.time()
                if name not in self.last_access_time or \
                   (current_time - self.last_access_time[name]) > config.ACCESS_COOLDOWN:
    ...
    def _on_detection_change(self):
        ...
        self._update_status(f"Detection: {self.current_detection.get().upper()}", "blue")
