...
class OpenFaceRecognizer:
    ...
    def __init__(self, distance_threshold: float = None):
        ...
        """
        self.distance_threshold = distance_threshold if distance_threshold is not None else 0.6
        ...
        if not FACE_RECOGNITION_AVAILABLE:
            print("[OpenFaceRecognizer] ERROR: Cannot initialize without face_recognition")
        
        if config.DEBUG:
            print(f"[OpenFaceRecognizer] Initialized with threshold: {self.distance_threshold}")
    ...
    def train(self, dataset_path: str = None) -> bool:
        ...
        try:
            ...
            if not user_dirs:
                print("[OpenFaceRecognizer] ERROR: No user directories found in dataset")
                return False
            ...
            for user_name in user_dirs:
                ...
                if len(image_files) < config.MIN_IMAGES_PER_PERSON:
                    print(f"[OpenFaceRecognizer] WARNING: {user_name} has only {len(image_files)} images "
                          f"(minimum: {config.MIN_IMAGES_PER_PERSON})")
                    continue
                
                print(f"[OpenFaceRecognizer] Processing {user_name}: {len(image_files)} images")
                ...
                for image_file in image_files[:config.MAX_IMAGES_PER_PERSON]:
                    ...
                    try:
                        ...
                        print(f"[OpenFaceRecognizer] WARNING: Skipping {image_file}: {img_error}")
            
            if not encodings:
                print("[OpenFaceRecognizer] ERROR: No valid encodings extracted")
                return False
            
            print(f"[OpenFaceRecognizer] Total encodings: {len(encodings)}, "
    ...
    def load_encodings(self) -> bool:
        ...
        try:
            ...
            print(f"[OpenFaceRecognizer] ✓ Encodings loaded: {len(names)} encodings, "
