...
class SFaceRecognizer:
    ...
    def __init__(self, distance_threshold: float = None):
        ...
        Args:
            distance_threshold: Cosine distance threshold (default 0.4)
        """
        self.distance_threshold = distance_threshold if distance_threshold is not None else 0.4
        ...
        if config.DEBUG:
            print(f"[SFaceRecognizer] Initialized with threshold: {self.distance_threshold}")
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if config.DEBUG:
                print(f"[SFaceRecognizer] ✓ Model loaded: {self.model_path}")
        ...
        try:
            ...
            for user_name in user_dirs:
                ...
                if len(image_files) < config.MIN_IMAGES_PER_PERSON:
                    print(f"[SFaceRecognizer] WARNING: {user_name} has only {len(image_files)} images "
                          f"(minimum: {config.MIN_IMAGES_PER_PERSON})")
                    continue
                
                print(f"[SFaceRecognizer] Processing {user_name}: {len(image_files)} images")
                ...
                for image_file in image_files[:config.MAX_IMAGES_PER_PERSON]:
                    ...
                    try:
                        ...
                        print(f"[SFaceRecognizer] WARNING: Skipping {image_file}: {img_error}")
            
            if not embeddings:
                print("[SFaceRecognizer] ERROR: No valid embeddings extracted")
                return False
            
            print(f"[SFaceRecognizer] Total embeddings: {len(embeddings)}, "
            ...
            if self.database.save_facenet_embeddings(names, embeddings):
                print("[SFaceRecognizer] ✓ Training completed and embeddings saved")
                return True
            else:
                print("[SFaceRecognizer] WARNING: Training completed but failed to save")
                return False
    ...
    def load_embeddings(self) -> bool:
        ...
        try:
            ...
            print(f"[SFaceRecognizer] ✓ Embeddings loaded: {len(names)} encodings, "
